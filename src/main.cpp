...
int64 nMintProofOfStake = MAX_MINT_PROOF_OF_STAKE;		// proof of stake rewards
...
// if someone created a tampered bootstrap.dat 
	...
	if (!lessAggressive)
	{
		...
		// by Simone: no one ever broadcast any checkpoint as BCF is fully decentralized coin, so this part is fully commented
...
unsigned char pchMessageStart[4] = { 0x97, 0xcb, 0xf5, 0xed };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BCFminer : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BCFminer : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBCFminer(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void BCFminer(CWallet *pwallet, bool fProofOfStake)
{
	...
	// by SIMONE: we must check this flag or this thread never exit when wallet is locked !
        ...
        printf("Running BCFminer with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBCFminer(void* parg)
{
    ...
    try
    {
        ...
        BCFminer(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBCFminer()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBCFminer()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBCFminer exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BCFminer threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBCFminer, pwallet))
                printf("Error: NewThread(ThreadBCFminer) failed\n");
            Sleep(10);
        }
    }
}
