...
void SkinsPage::getlist()
{
  ...
  statusLabel->setText("<b>" + tr("Downloading themes from bitcoinfast.co...") + "</b>");
  latestNetError = "";
...
// calculate the md5 of remote and local files
	...
	// get the local MD5
	    QString filename = inipath + "/list.txt";
		QFile file(filename);
...
void SkinsPage::getListFinished(QNetworkReply* reply)
{
	...
	// saves also the descriptor, will be used to check for an new version
		download((QString)"http://themes.bitcoinfast.co/themes/list.txt");
